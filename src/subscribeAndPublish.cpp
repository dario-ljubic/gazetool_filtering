...
{   
    // publish to a topic
    pub = n.advertise<gazetool::GazeHyps>("gazeHyps_filtered", 1); //TODO: check the necessary queue size
    
    // name of the topic has to be the same as the name of the topic to which gazetool is publishing unfiltered data
    sub = n.subscribe("gazeHyps_raw", 1, &subscribeAndPublish::callback, this); //TODO: check the necessary queue size, currently 2.5 sec of data is preserved
...
//         if (nanHorNum > 0) nanHorNum = nanHorNum - 1;
    ...
    if (writeToFile) writeObj.dumpEst(logFile, output);
}
...
void subscribeAndPublish::gazeFilter()
{
    ...
    Tpass = 0.3; // filter parameter, change for different pass frequencies!
...
void subscribeAndPublish::run(bool write, const char* path){
